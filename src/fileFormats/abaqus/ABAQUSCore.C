...
)
{
    ...
    while (is.peek() != '*' && is.peek() != EOF)
    {
        ...
        const label ansysFaceNum = (*faceIdIter)[sideNum - 1];
        ...
        switch (elemTypes_[index])
        {
            ...
            {
                elemNodes = labelList(connect, tet.modelFaces()[ansysFaceNum]);
                break;
            }
            case shapeType::abaqusPrism:
            {
                elemNodes = labelList(connect, prism.modelFaces()[ansysFaceNum]);
                break;
            }
            case shapeType::abaqusHex:
            {
                elemNodes = labelList(connect, hex.modelFaces()[ansysFaceNum]);
                break;
            }
            default:
                break;
