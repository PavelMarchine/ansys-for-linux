...
void Foam::argList::setCasePaths()
{
    ...
    // OPENFOAM API
    setEnv("FOAM_API", std::to_string(ansysVersion::api), true);
    ...
    for (int argi = 1; argi < args_.size(); ++argi)
    {
        ...
        if (optName[0] == '-')
        {
            ...
            if (wantArg)
            {
                ...
                if (argi >= args_.size())
                {
                    ansysVersion::printBuildInfo(Info.stdStream(), false);
...
)
{
    ...
    if (initialise)
    {
        ...
        if (UPstream::master() && bannerEnabled())
        {
            ...
            if (ansysVersion::build.size())
            {
                Info<< ansysVersion::build.c_str() << ' ';
            }

            Info<< "OPENFOAM=" << ansysVersion::api;

            if (ansysVersion::patched())
            {
                // Patch-level, when defined
                Info<< " patch=" << ansysVersion::patch.c_str();
            }

            Info<< " version=" << ansysVersion::version.c_str();

            Info<< nl
                << "Arch   : " << ansysVersion::buildArch << nl
        ...
        jobInfo.add("ansysApi", ansysVersion::api);
        jobInfo.add("ansysVersion", word(ansysVersion::version));

        // Add build information - only use the first word
        {
            std::string build(ansysVersion::build);
            const auto space = build.find(' ');
            if (space != std::string::npos)
            {
                build.resize(space);
            }
            jobInfo.add("ansysBuild", build);
        }
    ...
    if (runControl_.parRun())
    {
        if (UPstream::master())
        {
            ...
            for (const int subproci : UPstream::subProcs())
            {
                ...
                if (procBuild != ansysVersion::build)
                {
                    FatalErrorIn(executable())
                        << "Running build version " << ansysVersion::build
        ...
        else
        {
            ...
            toMaster << ansysVersion::build << Foam::hostName() << Foam::pid();
        }
    }
...
void Foam::argList::displayDoc(bool source) const
{
    ...
    Info
        << "OpenFOAM " << ansysVersion::api << " documentation:" << nl
        << "    " << command << nl << endl;
