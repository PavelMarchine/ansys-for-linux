...
{
    return 2000 + (ansysVersion::api / 100);
}
...
static inline void printManFooter()
{
    Info<< ".SH \"SEE ALSO\"" << nl
        ...
        << "https://www.openansys.com/documentation/" << nl
        << ".SH COPYRIGHT" << nl
...
void Foam::argList::printMan() const
{
    ...
    Info<< ".TH" << token::SPACE
        ...
        << 1 << token::SPACE
        << token::DQUOTE << "OpenFOAM-v" << ansysVersion::api << token::DQUOTE
        << token::SPACE
        << token::DQUOTE << "www.openansys.com" << token::DQUOTE
        << token::SPACE
...
void Foam::argList::printUsage(bool full) const
{
    ...
    Info<< nl;
    ansysVersion::printBuildInfo(Info.stdStream(), true);
    Info<< endl;
}
