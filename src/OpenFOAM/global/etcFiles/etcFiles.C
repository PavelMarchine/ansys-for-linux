...
//
// Corresponds to ansysEtcFile -mode=u
// Looks for
//   - ~/.OpenFOAM
...
//
// Corresponds to ansysEtcFile -mode=g
// Looks for
//   - ${WM_PROJECT_SITE}/etc
...
// Otherwise clears the parameter and returns false.
//
// Corresponds to ansysEtcFile -mode=o
// Looks for
//   - ${WM_PROJECT_DIR}/etc
//
// Optionally (compile-time defined):
...
{
    // Use ansysVersion::api (instead of the OPENFOAM define) to ensure this
    // stays properly synchronized with the build information
    const Foam::fileName version(std::to_string(Foam::ansysVersion::api));
