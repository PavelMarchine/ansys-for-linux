...
bool Foam::error::warnAboutAge(const int version) noexcept
{
    ...
    return ((version > 0) && (version < ansysVersion::api));
}
...
{
    // No warning for 0 (unversioned) or -ve values (silent versioning).
    // Also no warning for (version >= ansysVersion::api), which
    // can be used to denote future expiry dates of transition features.

    const bool old = ((version > 0) && (version < ansysVersion::api));
    ...
    if (old)
    {
        ...
        (
            // YYMM -> months
            (12 * (ansysVersion::api/100) + (ansysVersion::api % 100))
...
void Foam::error::write(Ostream& os, const bool withTitle) const
{
    ...
    {
        os  << title().c_str()
            << "(openansys-" << ansysVersion::api;

        if (ansysVersion::patched())
        {
            // Patch-level, when defined
            os  << " patch=" << ansysVersion::patch.c_str();
        }
        os  << ')' << nl;
    }
    os  << message().c_str();
