...
|   \\  /    A nd           | Website:  www.openansys.com                      |
|    \\/     M anipulation  |                                                 |
\*---------------------------------------------------------------------------*/
...
Foam::IOobject::writeBanner(Ostream& os, const bool noSyntaxHint)
{
    ...
    if (!*paddedVersion)
    {
        ...
        const std::string apiValue(std::to_string(Foam::ansysVersion::api));
...
)
{
    ...
    writeHeaderEntry(os, "arch", ansysVersion::buildArch);
...
)
{
    ...
    dict.set("arch", ansysVersion::buildArch);
