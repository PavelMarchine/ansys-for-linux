...
void Foam::boundaryMesh::readTriSurface(const fileName& fName)
{
    ...
    label ansysRegion = regionToBoundaryPatch[surfRegion];

    Pout<< "Surface region " << surfRegion << " becomes boundary patch "
        << ansysRegion << " with name " << patches_[ansysRegion].name() << endl;
    ...
    forAll(indices, indexI)
    {
        ...
        if (tri.region() != surfRegion)
        {
            ...
            boundaryPatch& bp = patches_[ansysRegion];

            bp.size() = bFacei - startFacei;
            bp.start() = startFacei;

            surfRegion = tri.region();
            ansysRegion = regionToBoundaryPatch[surfRegion];

            Pout<< "Surface region " << surfRegion << " becomes boundary patch "
                << ansysRegion << " with name " << patches_[ansysRegion].name()
                << endl;
