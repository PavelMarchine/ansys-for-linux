...
int main(int argc, char *argv[])
{
    ...
    forAll(cellVerts, celli)
    {
        ...
        if (ansysVerts.found(-1))
        {
            FatalErrorInFunction
                << "Cell " << celli
                << " unv vertices " << cellVerts[celli]
                << " has some undefined vertices " << ansysVerts
                << abort(FatalError);
        }

        // Bit nasty: replace vertex list.
        cellVerts[celli].transfer(ansysVerts);
    }
    ...
    {
        labelList ansysVerts(renumber(unvToFoam, boundaryFaces[bFacei]));
        ...
        // Bit nasty: replace vertex list.
        boundaryFaces[bFacei].transfer(ansysVerts);
    }
    ...
    if (dofVertIndices.size())
    {
        ...
        forAll(dofVertIndices, patchi)
        {
            const labelList& ansysVerts = dofVertIndices[patchi];
            dofGroups[patchi].insert(ansysVerts);
        }
