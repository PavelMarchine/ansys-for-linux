...
bool Foam::conformalVoronoiMesh::reconformToSurface() const
{
    ...
    (
        runTime_.timeIndex()
      % ansysyHexMeshControls().surfaceConformationRebuildFrequency() == 0
    )
    {
        return true;
    }
...
//            (
//                ansysyHexMeshControls().nearBoundaryDistanceCoeff()
//               *targetCellSize(pt)
//            );
//
//            pointIndexHit pHit;
...
void Foam::conformalVoronoiMesh::buildSurfaceConformation()
{
    ...
    const scalar edgeSearchDistCoeffSqr =
        ansysyHexMeshControls().edgeSearchDistCoeffSqr();

    const scalar surfacePtReplaceDistCoeffSqr =
        ansysyHexMeshControls().surfacePtReplaceDistCoeffSqr();
    ...
    label maxIterations = ansysyHexMeshControls().maxConformationIterations();

    scalar iterationToInitialHitRatioLimit =
        ansysyHexMeshControls().iterationToInitialHitRatioLimit();
...
) const
{
    ...
    else
    {
        ...
        (
           ...
           *ansysyHexMeshControls().pointPairDistanceCoeff()
        );
