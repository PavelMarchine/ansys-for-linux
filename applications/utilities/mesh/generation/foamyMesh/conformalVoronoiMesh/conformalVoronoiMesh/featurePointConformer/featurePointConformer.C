...
) const
{
    ...
    forAll(masterPoints, pI)
    {
        ...
        (
            ...
            (
                masterPt,
                ansysyHexMesh_.vertexCount() + pts.size(),
                masterType,
                Pstream::myProcNo()
            )
        );
...
) const
{
    ...
    (
        (
            Pstream::parRun()
         && !ansysyHexMesh_.decomposition().positionOnThisProcessor(featPt)
        )
    ...
    const scalar ppDist = ansysyHexMesh_.pointPairDistance(featPt);
...
{
    if (ansysyHexMeshControls_.mixedFeaturePointPPDistanceCoeff() < 0)
    {
        Info<< nl
    ...
    forAll(feMeshes, i)
    {
        ...
        )
        {
            ...
            if (ansysyHexMeshControls_.specialiseFeaturePoints())
            {
                specialisedSuccess =
            ...
            if (!specialisedSuccess && ansysyHexMeshControls_.edgeAiming())
            {
...
//                createFeaturePoints(feMesh, ptI, pts, types);
                ...
                const scalar edgeGroupDistance =
                    ansysyHexMesh_.mixedFeaturePointDistance(pt);
                ...
                forAll(pEds, e)
                {
                    ...
                    ansysyHexMesh_.createEdgePointGroup(feMesh, edgeHit, pts);
                }
            }
        }
    }
...
//            ptI < feMesh.nonFeatureStart();
        ...
        if (ansysyHexMeshControls_.guardFeaturePoints())
        {
            for
            (
                //label ptI = feMesh.convexStart();
...
(
    const conformalVoronoiMesh& ansysyHexMesh
)
:
    ansysyHexMesh_(ansysyHexMesh),
    ansysyHexMeshControls_(ansysyHexMesh.ansysyHexMeshControls()),
    geometryToConformTo_(ansysyHexMesh.geometryToConformTo()),
    featurePointVertices_(),
    ftPtPairs_(ansysyHexMesh)
{
    Info<< nl
    ...
    Info<< incrIndent
        ...
        << ansysyHexMeshControls_.circulateEdges().c_str() << nl
        << indent << "Guarding feature points is: "
        << ansysyHexMeshControls_.guardFeaturePoints().c_str() << nl
        << indent << "Snapping to feature points is: "
        << ansysyHexMeshControls_.snapFeaturePoints().c_str() << nl
        << indent << "Specialising feature points is: "
        << ansysyHexMeshControls_.specialiseFeaturePoints().c_str()
        << decrIndent
        << endl;
    ...
    if (ansysyHexMeshControls_.objOutput())
    {
