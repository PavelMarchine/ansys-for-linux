...
)
{
    ...
    patchNames[patchNames.size() - 1] = "ansysyHexMesh_defaultPatch";
...
//        }
//
//        if (++iterI > ansysyHexMeshControls().maxCollapseIterations())
//        {
...
////
////        if (fC > ansysyHexMeshControls().filterCountSkipThreshold())
////        {
...
//    (
//        degToRad(ansysyHexMeshControls().surfaceStepFaceAngle())
//    );
//
//    for
//    (
...
//
//            if (maxFC > ansysyHexMeshControls().filterCountSkipThreshold())
//            {
...
void Foam::conformalVoronoiMesh::checkCellSizing()
{
    ...
    const dictionary& dict
        = ansysyHexMeshControls().ansysyHexMeshDict();
...
)
{
    ...
    if (ansysyHexMeshControls().guardFeaturePoints())
    {
        for
        (
            Delaunay::Finite_vertices_iterator vit = finite_vertices_begin();
...
//        }
        ...
        if (!cit->hasFarPoint())
        {
            ...
            if (ansysyHexMeshControls().snapFeaturePoints())
            {
                if (cit->featurePointDualVertex())
                {
...
) const
{
    ...
    patchNames[defaultPatchIndex] = "ansysyHexMesh_defaultPatch";
