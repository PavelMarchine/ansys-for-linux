...
)
{
    ...
    if (ansysyHexMeshControls().objOutput() && !fName.empty())
    {
...
void Foam::conformalVoronoiMesh::insertInitialPoints()
{
    ...
    if (ansysyHexMeshControls().objOutput())
    {
...
void Foam::conformalVoronoiMesh::buildCellSizeAndAlignmentMesh()
{
    ...
    const dictionary& motionControlDict
        = ansysyHexMeshControls().ansysyHexMeshDict().subDict("motionControl");
    ...
    if (ansysyHexMeshControls().writeCellShapeControlMesh())
    {
        //cellSizeMesh.writeTriangulation();
        cellSizeMesh.write();
    }

    if (ansysyHexMeshControls().printVertexInfo())
    {
...
(
    const Time& runTime,
    const dictionary& ansysyHexMeshDict,
...
:
    ...
    ansysyHexMeshControls_(ansysyHexMeshDict),
    allGeometry_
    (
        IOobject
        (
        ...
        ansysyHexMeshDict.subDict("geometry"),
        ansysyHexMeshDict.getOrDefault("singleRegionName", true)
    ),
    ...
    (
        ...
        rndGen_,
        allGeometry_,
        ansysyHexMeshDict.subDict("surfaceConformation")
    ),
    decomposition_
    (
        Pstream::parRun()
      ? new backgroundMeshDecomposition
        (
            ...
            rndGen_,
            geometryToConformTo_,
            ansysyHexMeshControls().ansysyHexMeshDict().subDict
            (
    ...
    (
        ...
        (
            ansysyHexMeshDict.subDict("initialPoints"),
            runTime_,
            rndGen_,
            geometryToConformTo_,
            cellShapeControl_,
    ...
    (
        relaxationModel::New
        (
            ansysyHexMeshDict.subDict("motionControl"),
            runTime_
        )
    ),
...
void Foam::conformalVoronoiMesh::initialiseForMotion()
{
    if (ansysyHexMeshControls().objOutput())
    {
        geometryToConformTo_.writeFeatureObj("ansysyHexMesh");
    }
    ...
    )
    {
        ...
        )
        {
            ...
            forAll(alignmentDirs, aD)
            {
                ...
                (
                    alignmentDotProd
                  > ansysyHexMeshControls().cosAlignmentAcceptanceAngle()
                )
                {
...
//                            )
                     ...
                     && rABMag
                      > ansysyHexMeshControls().insertionDistCoeff()
                       *targetCellSize
                     && faceArea
                      > ansysyHexMeshControls().faceAreaRatioCoeff()
                       *targetFaceArea
                     && alignmentDotProd
                      > ansysyHexMeshControls().cosInsertionAcceptanceAngle()
                    )
                    {
                        // Point insertion
                        if
                    ...
                    (
                     ...
                     && rABMag
                      < ansysyHexMeshControls().removalDistCoeff()
                       *targetCellSize
                    )
                    {
                        // Point removal
    ...
    if (ansysyHexMeshControls().objOutput() && time().writeTime())
    {
...
void Foam::conformalVoronoiMesh::checkCoPlanarCells() const
{
    ...
    {
        const scalar quality = ansysyHexMeshChecks::coplanarTet(cit, 1e-16);
