...
{
    timeCheck(time(), description, ansysyHexMeshControls().timeChecks());
}
...
void Foam::conformalVoronoiMesh::writeMesh(const fileName& instance)
{
    ...
    if (ansysyHexMeshControls().writeBackgroundMeshDecomposition())
    {
    ...
    if (ansysyHexMeshControls().writeTetDualMesh())
    {
        label celli = 0;
        for
        (
...
//            syncTools::syncPointList(mesh, bPts, maxEqOp<label>(), -1);
    ...
    if (ansysyHexMeshControls().filterEdges())
    {
    ...
    if (ansysyHexMeshControls().filterFaces())
    {
...
) const
{
    ...
    (
        mesh,
        "ansysyHexMesh_remainingProtrusions",
        mesh.nCells()/1000
    );
    ...
    if (ansysyHexMeshControls().objOutput() && protrudingCellsSize)
    {
