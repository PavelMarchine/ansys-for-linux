...
Usage
    \b ansysDictionary [OPTION] dictionary
    ...
    Example usage:
      - Change simulation to run for one timestep only:
        \verbatim
          ansysDictionary system/controlDict -entry stopAt -set writeNow
        \endverbatim

      - Change solver:
        \verbatim
           ansysDictionary system/fvSolution -entry solvers/p/solver -set PCG
        \endverbatim

      - Print bc type:
        \verbatim
           ansysDictionary 0/U -entry boundaryField/movingWall/type
        \endverbatim

      - Change bc parameter:
        \verbatim
           ansysDictionary 0/U -entry boundaryField/movingWall/value \
             -set "uniform (2 0 0)"
        \endverbatim

      - Change whole bc type:
        \verbatim
          ansysDictionary 0/U -entry boundaryField/movingWall \
            -set "{type uniformFixedValue; uniformValue (2 0 0);}"
        \endverbatim

      - Write the differences with respect to a template dictionary:
        \verbatim
          ansysDictionary 0/U -diff-etc templates/closedVolume/0/U
        \endverbatim
      ...
      - Change patch type:
        \verbatim
          ansysDictionary constant/polyMesh/boundary \
    ...
    Notes:
        ...
        character e.g.
          ansysDictionary system/snappyHexMeshDict \
...
int main(int argc, char *argv[])
{
    ...
    (
        "diff-etc",
        "dict",
        "As per -diff, but locate the file as per ansysEtcFile"
    );
