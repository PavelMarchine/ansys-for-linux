...
{
    Info<< "Extract: " << str << " =>"
        << " label: " << ansysVersion::labelByteSize(str) << " bytes"
        << " scalar: " << ansysVersion::scalarByteSize(str) << " bytes"
        << nl;
}
...
int main()
{
    ...
    Info
        ...
        << "patched?  = " << Switch(ansysVersion::patched()) << nl
        << "api       " << ansysVersion::api << nl
        << "patch     " << ansysVersion::patch << nl
        << "version   " << ansysVersion::version << nl
        << "build     " << ansysVersion::build << nl
        << "buildArch " << ansysVersion::buildArch << nl;

    Info
        << "\nTypes" << nl
        << "version   " << typeid(ansysVersion::version).name() << nl
        << "build     " << typeid(ansysVersion::build).name() << nl
        << "buildArch " << typeid(ansysVersion::buildArch).name() << nl
    ...
    Info
        ...
        << "namespace " << name(&(ansysVersion::version[0])) << nl;
